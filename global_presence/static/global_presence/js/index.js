$(document).ready(function(){
    ...
    // loads thumbnails based on the screen resolution
    ...
    hide_handler = function(){
        $('.square').hide();
        $('div.all img').removeClass('collage_selected');
    };
    show_handler = function(){
        hide_handler();
        $(this).siblings('.square').show('slow');
        $(this).addClass('collage_selected');
    };
    $('div.all img').on('mouseover', show_handler);
    ...
    // show/hide team preview
    if (!window.mobilecheck()) {
        $('.map_button').on("mouseenter", function(){
            $('.team_preview').hide(1);
            $('.team_preview#'+this.id).fadeIn(500);
        });
    };
